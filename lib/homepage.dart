...
import 'package:pass_manager/searchpage.dart';
...
class _homepageState extends State<homepage> {
    ...
    ref.child('publickey').once().then((DataSnapshot snap) async {
      ...
      setState(() async {
        final String encryptkey =
            await cryptor.generateKeyFromPassword(_publicKey, _privateKey);
              ...
              setState(() {
                ...
                for (var x in allkeys) {
                  ref
                    ...
                    var data = snap.value;
                    var newemail =
                        await cryptor.decrypt(data['email'], encryptkey);
                    var newdomain =
                        await cryptor.decrypt(data['domain'], encryptkey);
                    var newpassword =
                        await cryptor.decrypt(data['password'], encryptkey);
                    constdata newdata =
                        new constdata(newemail, newdomain, newpassword, x);
    ...
    Future.delayed(const Duration(seconds: 5), () {
      setState(() {
        ref.child('$_userid').once().then((DataSnapshot snap) async {
          ...
          await store.write(
              key: 'offline-data', value: '${JSON.jsonEncode(snap.value)}');
          await store.write(
              key: 'offline-key', value: '${JSON.jsonEncode(newkeylist)}');
        });
      });
    });
  }
  ...
  Future Message_Data_To_Update(
      var domain, var email, var password, var timestamp) async {
      ...
      child: Scaffold(
        ...
        floatingActionButton: SpeedDial(
          ...
          children: [
            ...
            SpeedDialChild(
              ...
              onTap: () {
                Navigator.push(context,MaterialPageRoute(builder: (context) => searchpage(data: allData)));
              },
              backgroundColor: Colors.deepPurpleAccent,
            ),
          ],
        ...
        body: allData.length == 0
            ...
            : new ListView.builder(
                ...
                itemBuilder: (_, index) {
                  return widgetUI(data: allData[index]);
                }),
      ),
    );
  }
  ...
  //               Message_Data_To_Update(domain, email, password, timestamp);
  //             },
  //             icon: new Icon(
  //               Icons.update,
  //               size: 30.0,
